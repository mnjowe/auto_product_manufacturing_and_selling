...
if(isset($_POST['buy_materials'])){
  ...
  if($lastInsertId){
    ...
    $sql = "UPDATE `tbl_stock_material` SET m_expected_date =  :arrival_date WHERE material_name = :material_id";
    $query = $dbconn->prepare($sql);
    $query->bindParam(':material_id', $material_id, PDO::PARAM_STR);
    $query->bindParam(':arrival_date', $arrival_date, PDO::PARAM_STR);
    $query->execute();
  ...
  <div class="card shadow mb-4">
    ...
    <div class="table-responsive">
      <?php
      $sql = "SELECT tbl_materials.id, tbl_stock_material.in_stock,  tbl_stock_material.m_expected_date, tbl_materials.material_name, tbl_materials.material_code, tbl_categories.category_description, tbl_suppliers.supplier_name
          ...
          <?php if($count > 0)
          {
            foreach($rows as $row) {
              ...
              <tr>
                ...
                <td>
                  <?php if($row->in_stock > 0 OR $row->m_expected_date == 0){
                    echo("");
                  }else {
                    // code...
                    echo ($row->m_expected_date);
                  };?>
                </td>              <td>
